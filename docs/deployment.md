...
Die Datei `config/supabase-config.json` muss in der Zielumgebung vorhanden sein, darf aber keine geheimen Service-Role-Keys enthalten. Die Datenbankstruktur wird über `supabase/schema.sql` und die Migrationen unter `supabase/migrations/` gepflegt.
